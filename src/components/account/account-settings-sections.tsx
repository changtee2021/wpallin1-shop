...
import { BankBookPanel } from "@/components/account/bank-book-panel";
...
import { SystemSettingsPanel } from "@/components/account/system-settings-panel";
...
export function AccountSettingsSections(props: AccountSettingsSectionsProps) {
  const { t } = useT();
  const sectionTitle =
    SETTINGS_SECTIONS.find((s) => s.id === props.section)?.label ?? "ตั้งค่า";
      ...
      <div className="min-w-0">
        <SettingsSection title={sectionTitle}>
          ...
          {props.section === "finance" ? (
            <FinanceSection {...props} t={t} />
          ) : null}
          {props.section === "address-tax" ? (
            <AddressTaxSection {...props} t={t} />
          ) : null}
          {props.section === "system" ? (
            <SystemSection {...props} t={t} />
          ) : null}
        </SettingsSection>
      </div>
    </div>
...
function PersonalSection({
  ...
  onSaveProfile,
  t,
}: AccountSettingsSectionsProps & { t: ReturnType<typeof useT>["t"] }) {
  return (
    <div className="space-y-4">
      <Card>
        <CardContent className="p-6">
          <ProfileAvatarPicker
            displayName={profile.fullName ?? "สมาชิก"}
            email={profile.email ?? ""}
            avatarUrl={profile.avatarUrl}
            onUploaded={() => onProfileRefresh()}
          />
        </CardContent>
      </Card>
...
function FinanceSection(
  props: AccountSettingsSectionsProps & {
  ...
  return (
    <div className="space-y-4">
            ...
            <p>
              ...
              {props.tierProgress.nextTierName ? (
                <>
                  {" "}
                  · อีก {formatPrice(
                    props.tierProgress.amountToNext ?? 0,
                  )} ถึง {props.tierProgress.nextTierName}
                </>
              ) : (
                " · ระดับสูงสุดแล้ว"
              )}
      ...
      <BankBookPanel
        bankAccount={props.profile.bankAccount}
        onSubmitted={props.onProfileRefresh}
      />
...
function AddressTaxSection(
  ...
  useEffect(() => {
    ...
    if (hash !== "section-tax-invoice") return;
  ...
  return (
    <div className="space-y-6">
      <div id="section-kyc" className="scroll-mt-24">
        <h3 className="mb-3 text-sm font-medium">
          {props.t("account.documents")} / KYC
        </h3>
      ...
      <div id="section-tax-invoice" className="scroll-mt-24">
        <h3 className="mb-3 text-sm font-medium">
          {props.t("account.taxInvoice")}
        </h3>
        <Card>
          <CardHeader>
            <CardTitle className="text-base">
                ...
                <CardContent className="flex flex-wrap items-start justify-between gap-3 p-4">
                  <div className="text-sm">
                    <div className="mb-1 flex flex-wrap items-center gap-2">
                      ...
                      {tax.isDefault && <Badge variant="secondary">หลัก</Badge>}
                    </div>
                    <p className="text-muted-foreground">
                      เลขภาษี {tax.taxId}
...
function SettingsSection({
  ...
  return (
    <section className="space-y-4">
      <h2 className={cn("border-b pb-2 text-lg font-semibold tracking-tight")}>
        {title}
      </h2>
      {children}
    </section>
